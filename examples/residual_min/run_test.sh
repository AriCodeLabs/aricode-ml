#!/bin/bash
# residual_min — packed-binary residual-add regression.
set -euo pipefail
cd "$(dirname "$0")"
SCRIPT_DIR="$(pwd)"
...
"$PY" "$PACK" \
    ...
    --embed --no-argmax --out resid_min > /dev/null

echo "[3/4] compiling..."
"$ARIC" resid_min.ari -o resid_min > /dev/null

echo "[4/4] comparing to PyTorch..."
./resid_min > /tmp/resid_min_out.txt
"$PY" -c "
import numpy as np
expected = np.fromfile('expected.f32', dtype=np.float32)
got = np.array([float(l) for l in open('/tmp/resid_min_out.txt') if l.strip()],
               dtype=np.float32)
...
print('  RESID_OK — packed binary matches PyTorch residual-FFN reference')
"
