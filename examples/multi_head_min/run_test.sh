#!/bin/bash
# multi_head_min — packed-binary multi-head attention regression.
set -euo pipefail
cd "$(dirname "$0")"
SCRIPT_DIR="$(pwd)"
...
"$PY" "$PACK" \
    ...
    --embed --no-argmax --out mha_min > /dev/null

echo "[3/4] compiling..."
"$ARIC" mha_min.ari -o mha_min > /dev/null

echo "[4/4] comparing to PyTorch..."
./mha_min > /tmp/mha_min_out.txt
"$PY" -c "
import numpy as np
expected = np.fromfile('expected.f32', dtype=np.float32)
got = np.array([float(l) for l in open('/tmp/mha_min_out.txt') if l.strip()],
               dtype=np.float32)
...
print('  MHA_OK — packed multi-head attention matches PyTorch reference')
"
