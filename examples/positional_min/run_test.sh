#!/bin/bash
# positional_min — packed-binary embedding + learned positional embedding.
set -euo pipefail
cd "$(dirname "$0")"
SCRIPT_DIR="$(pwd)"
...
echo "[1/4] generating synthetic embedding+pos checkpoint..."
"$PY" make_synth.py
...
"$PY" "$PACK" \
    ...
    --embed --no-argmax --out pos_min > /dev/null

echo "[3/4] compiling..."
"$ARIC" pos_min.ari -o pos_min > /dev/null

echo "[4/4] comparing to PyTorch..."
./pos_min > /tmp/pos_min_out.txt
"$PY" -c "
import numpy as np
expected = np.fromfile('expected.f32', dtype=np.float32)
got = np.array([float(l) for l in open('/tmp/pos_min_out.txt') if l.strip()],
               dtype=np.float32)
...
print('  POS_OK — packed embedding + positional_embedding matches PyTorch')
"
